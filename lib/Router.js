...
Router.prototype.manage = function (req, res) {
  ...
  if (callback && req.method !== 'GET') {
    ...
    req.on('end', () => {
      _callMiddleware(0, this.middlewares, callback, req, res)
    })
  } else if (callback) {
    _callMiddleware(0, this.middlewares, callback, req, res)
  } else {
 ...
 * @param {object} res.
 */
function _callMiddleware (index, middlewares, callback, req, res) {
  if (middlewares[index]) {
    middlewares[index](req, res, () => _callMiddleware(index + 1, middlewares, callback, req, res))
  } else {
    callback(req, res)
  }
}
